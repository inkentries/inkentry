...
use inkentry_core::indexer::{Chunk, SourceParser, set_chunk_token_cap};
use inkentry_embed::NativeEmbedder;
use tokenizers::Tokenizer;

/// Matches the query format `inkentry-server` uses for code search (see
...
/// generated-file markers), mirroring `inkentry-cli`'s `collect_files` (not
/// reusable directly: it lives in a crate `inkentry-core` doesn't depend on).
