...
impl CloudSyncClient {
    ...
    /// (a UUID for cloud-api, or a slug for an OSS inkentry-server); it is
    ...
    /// `crates/inkentry-server/src/relay.rs`, the only place in the workspace
    ...
    /// `pull_and_apply_since` in inkentry-cli), calling again with the cursor
    ...
    /// having nothing to pull, not an error: a inkentry-server-backed project
...
mod tests {
    ...
    fn unit_vec_896() -> Vec<f32> {
        let n = inkentry_core_embedding_dim();
        vec![1.0 / (n as f32).sqrt(); n]
    }

    fn inkentry_core_embedding_dim() -> usize {
    ...
    /// `pull_and_apply_since` in inkentry-cli) depends on this distinction to
