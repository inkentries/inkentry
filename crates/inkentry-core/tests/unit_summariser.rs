//! Tests for the UUID-keyed delimiter in `inkentry_core::indexer::summariser`.
//!
...
use inkentry_core::llm::{LlmBackend, Message};
...
async fn delimiter_uuid_differs_between_consecutive_calls() {
    ...
    // First call.
    let results_a = inkentry_core::indexer::summariser::summarise_batch(&llm_a, &chunks_a)
        .await
    ...
    let results_b = inkentry_core::indexer::summariser::summarise_batch(&llm_b, &chunks_b)
        .await
...
async fn all_chunks_in_one_batch_share_the_same_uuid() {
    ...
    let results = inkentry_core::indexer::summariser::summarise_batch(&llm, &chunks)
        .await
