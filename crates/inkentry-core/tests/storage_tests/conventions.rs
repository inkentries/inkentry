...
fn rust_functions_snake_case() {
    ...
    let records = inkentry_core::conventions::rules::rust::extract(&refs, "rust", 0);
...
fn ts_functions_camel_case() {
    ...
    .collect();
    let refs: Vec<&ChunkSummary> = chunks.iter().collect();
    let records = inkentry_core::conventions::rules::typescript::extract(&refs, "typescript", 0);
    let r = find_record(&records, "naming.functions").expect("naming.functions record");
    assert!(
...
fn list_conventions_wrapper_converts_correctly() {
    ...
    let records = inkentry_core::conventions::list_conventions(&db, None).unwrap();
...
fn extractor_emits_low_evidence_raw_records() {
    ...
    let refs: Vec<&ChunkSummary> = chunks.iter().collect();
    let raw = inkentry_core::conventions::rules::rust::extract(&refs, "rust", 0);
