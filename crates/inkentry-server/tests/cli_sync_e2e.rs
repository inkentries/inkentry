...
//! - since roundtrip (`?since_id=`, `inkentry sync`'s pull-half contract):
//!   `CloudSyncClient::pull_since` — the same client code `inkentry sync`
...
// ── `inkentry sync` pull half: CloudSyncClient::pull_since ──────────────────
