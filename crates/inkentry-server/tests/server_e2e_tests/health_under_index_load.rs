...
async fn spawn_server(embedder: inkentry_server::EmbedderSlot) -> String {
    common::register_sqlite_vec();
    let db = inkentry_server::db::ServerDb::open(Path::new(":memory:"), DIM, "test-model")
...
async fn health_and_memory_search_stay_usable_throughout_a_real_index() {
    ...
    let seed = client
        ...
        .json(&json!({
            ...
            "vector": vec![1.0_f32 / (DIM as f32).sqrt(); DIM],
