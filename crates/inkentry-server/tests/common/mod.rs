...
/// Open an in-memory `inkentry_server::db::ServerDb` for tests.
///
/// Calls `register_sqlite_vec()` automatically.
pub fn open_test_server_db(dim: usize) -> inkentry_server::db::ServerDb {
    register_sqlite_vec();
    inkentry_server::db::ServerDb::open(std::path::Path::new(":memory:"), dim, "test-model")
        .expect("failed to open in-memory server database")
}

/// Build a minimal `AppState` backed by an in-memory DB for integration tests.
pub fn make_test_state(dim: usize, auth_key: Option<String>) -> inkentry_server::AppState {
    let db = open_test_server_db(dim);
    let instance_id = db.get_or_create_instance_id().expect("instance_id in test");
    inkentry_server::AppState {
        db: std::sync::Arc::new(tokio::sync::Mutex::new(db)),
        auth: std::sync::Arc::new(inkentry_server::auth::ApiKeyAuth::new(auth_key)),
