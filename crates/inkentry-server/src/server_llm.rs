...
pub fn check_llm_transport(llm_url: &str, has_key: bool) -> Result<()> {
    ...
    inkentry_core::config::validate_transport_url(llm_url).map_err(|e| {
        anyhow::anyhow!(
...
impl inkentry_core::llm::LlmBackend for ServerLlm {
    async fn generate(
        &self,
        ...
        struct Delta {
            ...
            // Reasoning models stream chain-of-thought here, as a sibling of
            // `content`. It is intermediate output, never the answer, so it is
            // parsed only to be dropped: forwarding it would corrupt a
...
mod tests {
    use super::*;
    use inkentry_core::llm::LlmBackend;
