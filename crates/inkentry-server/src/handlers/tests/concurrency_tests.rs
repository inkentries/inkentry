...
impl inkentry_core::embeddings::EmbeddingBackend for CancelAwareEmbedder {
...
#[async_trait::async_trait]
impl inkentry_core::embeddings::EmbeddingBackend for QueuedCancelEmbedder {
    async fn embed(&self, texts: &[&str]) -> anyhow::Result<Vec<Vec<f32>>> {
        self.embed_with_cancel(texts, Arc::new(std::sync::atomic::AtomicBool::new(false)))
            .await
    }
...
#[async_trait::async_trait]
impl inkentry_core::embeddings::EmbeddingBackend for GatedEmbedder {
    async fn embed(&self, texts: &[&str]) -> anyhow::Result<Vec<Vec<f32>>> {
