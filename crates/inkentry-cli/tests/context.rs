...
// ── preserve-the-capability guard ───────────────────────────────────────────────
//
// This change ADDS the intent/overlap surface to `context`; it must not remove
// the equivalent surface from `check`, which stays until a separate follow-up
// deletes it. Guard that `check` still lists active sessions and warns on
// overlap, so the capability is never lost in the gap between the two changes.

#[test]
fn check_still_surfaces_active_intents_and_overlap() {
    let tmp = TempDir::new().expect("create temp dir");
    let proj = tmp.path();
    std::fs::write(proj.join("main.rs"), "fn main() {}\n").expect("write main.rs");

    // Create the index so `check` has something to report against.
    inkentry_bin()
    ...
    // Seed an intent claiming main.rs (still non-git: the git-notes carrier is a
    // no-op, memory lands in .inkentry/memory.db).
    inkentry_bin()
        .env("INKENTRY_NO_SERVER", "1")
        .current_dir(proj)
        .args([
    ...
    // Make main.rs a tracked-and-modified file so the overlap fires.
    init_git_repo(proj);
    {
        use std::io::Write as _;
        let mut fh = std::fs::OpenOptions::new()
            .append(true)
            .open(proj.join("main.rs"))
            .expect("open main.rs");
        writeln!(fh, "// touched").expect("append");
    }
    ...
    let out = inkentry_bin()
        ...
        .arg("check")
        .assert()
        // main.rs changed since indexing => stale => exit 1; the intent block
        // still prints before the non-zero exit.
        .code(1)
        .get_output()
        .stdout
        .clone();
    ...
    assert!(
        text.contains("Active agent sessions:"),
        "check must still list active sessions; got:\n{text}"
    );
    assert!(
        text.contains("⚠  Overlap: main.rs is listed in an active intent"),
        "check must still warn on overlap; got:\n{text}"
    );
}
