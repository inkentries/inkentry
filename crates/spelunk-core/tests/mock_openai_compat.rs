//! Tests for the OpenAI-compatible embedding backend using wiremock.
//!
//! These tests spin up a local mock HTTP server so no real API server
...
async fn embed_returns_vectors_from_server() {
    ...
    let embedder = OpenAiCompatEmbedder::load(&cfg).await.unwrap();
