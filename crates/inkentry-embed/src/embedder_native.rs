...
impl NativeEmbedder {
    ...
    /// (`inkentry-server` resolves the artifacts via Hugging Face Hub first).
    ///
...
mod tests {
    ...
    // ignored network tests in inkentry-server's `embed_hub`.
    ...
    // the `token_cap()` proof) live in inkentry-server's `embed_hub`, which can
