...
- **Multi-hop exploration** — loop `inkentry search` + `inkentry search <symbol> --graph` + `inkentry chunks` yourself for questions that span files (see [the skill](https://github.com/inkentries/agent-plugin/blob/main/skills/inkentry/SKILL.md)).
...
`1` would say "no entries" or "nothing to push" when the truth is that there was
nothing there to read or push from. `push --source <path>` is held to the same
rule: a source the caller named and got wrong is where conjuring a store is
least defensible, since the run would report an empty delta for a file the
caller believes holds their memory.

`pull` and the porcelain `inkentry sync` are the deliberate exceptions, and they
bootstrap without `inkentry init`: both **receive** memory, and creating the
store is how a fresh checkout first gets its team's memory. They create it
rather than refusing. (`sync` still needs a `.inkentry/` directory to resolve
the project; it is the `memory.db` inside it that sync creates.)

Under `cloud_first` with a configured `server_url` the team server owns the
store and the local path is a placeholder nothing opens, so no command refuses
on an absent local file in that mode.
...
| `graph-edges` | `inkentry plumbing graph-edges --file <f> \| --symbol <s>` | Emit code graph edges (imports, calls, extends) for a file or symbol as JSONL. At least one of `--file` or `--symbol` is required. Exits `1` if the file or symbol is indexed but has no edges, and `2` if `--file` names a path the index does not hold (the path is reported on stderr). |
| `read-memory` | `inkentry plumbing read-memory [--kind <k>] [--id <uuid>] [--limit N]` | Emit memory entries as JSONL. Filter by kind (`decision`, `question`, `note`, etc.) or fetch a single entry by id. |
...
| `push` | `inkentry plumbing push [--source <path>] [--include-archived]` | One-way local to server memory push for seeding or CI. **Writes to the team server.** Emits one report object: `{attempted, created, skipped, failed, already_synced, edges_pushed, without_local_vector, embedded_locally, interrupted}`. Exit `0` when ≥1 entry was created; exit `1` on an empty delta (nothing local to push, or everything already present), and the report is still emitted; exit `2` when the run did not complete (no local memory store at all, no `server_url`/`project_id`, unreachable/auth, a total failure where nothing landed, or an interruption), with stdout empty and a diagnostic on stderr. An absent store is refused rather than reported as an empty delta, and nothing is created: push only sends, so a push from a store that does not exist has nothing behind it. Run `inkentry init` first (see [Missing stores](#missing-stores)). `--source` pushes from a specific `memory.db`; `--include-archived` propagates tombstones. For everyday two-way convergence use `inkentry sync`. |
| `pull` | `inkentry plumbing pull` | One-way server to local memory delta pull (cursored on the last-synced remote id). **Reads from the team server, writes locally.** Emits one report object: `{applied, embedded_locally, without_local_vector}`. Applied entries are also embedded locally so `inkentry search --only-memory` can surface them; `without_local_vector` counts the ones no embedder could be reached for, which the next pull retries. Exit `0` when ≥1 entry was applied; exit `1` on an empty delta (nothing new), and the report is still emitted; exit `2` on a setup/network/auth error, with stdout empty and a diagnostic on stderr. Unlike `push` and `read-memory`, an absent local store is **created** rather than refused: pull receives, and creating the store is how a fresh checkout first gets its team's memory (see [Missing stores](#missing-stores)). For everyday two-way convergence use `inkentry sync`. |
