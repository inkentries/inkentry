...
| `publish-notes` | `spelunk plumbing publish-notes [remote] [--best-effort]` | Publish memory notes (`refs/notes/spelunk`) to `remote` (default `origin`): fetch onto the tracking ref, union-merge with `cat_sort_uniq`, push. Never force-pushes. **Writes and performs network I/O.** `--best-effort` warns on stderr and exits `0` instead of failing, which is what the pre-push hook uses. Reach for `spelunk hooks install --pre-push` rather than calling this directly. |
