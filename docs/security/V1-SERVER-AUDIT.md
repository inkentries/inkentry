...
interface/port and points at the loopback-only / TLS-front guidance. There is no
override; the only supported posture for a shared server is to bind loopback and
terminate TLS in a front proxy. Covered by unit test
...
| Check | Status |
|---|---|
| Server refuses to start if `JWT_SECRET` is absent or < 32 bytes | N/A (cloud-only). The OSS server has no JWT; auth is the shared `SPELUNK_SERVER_KEY`. The applicable startup guard is `main.rs::check_bind_safety`, ☑ implemented: it refuses a non-loopback plaintext bind **unconditionally** in both the keyless case (open server) and the keyed case (bearer key in cleartext), naming the interface and pointing at the loopback/TLS-front guidance. Neither refusal has an opt-out (oss^79). |
