...
- [Portable dump format](dump-format.md): the spec `inkentry import` reads, for crossing a pre-1.0.0 `spelunk-export` dump into inkentry
- [Upgrading](upgrading.md): 1.0.x to 1.1.0 needs nothing but the new binaries; the earlier move to 1.0.0 does not open an older store, so it is a repository-wide event your team plans together
