...
Every release publishes two independent things to check against, and they answer
different questions.

**`SHA256SUMS`** answers *did I receive the file that was uploaded* — it catches a
truncated download or a tampered mirror. Download it alongside the archive:

```bash
curl -LO https://github.com/inkentries/inkentry/releases/download/<version>/SHA256SUMS
sha256sum --ignore-missing -c SHA256SUMS   # shasum -a 256 --ignore-missing -c on macOS
```

**Build provenance** answers the stronger question — *was this file built by
inkentry's release workflow, from this repository* — and is what to check if you
care that a release itself has not been swapped. It needs the [`gh`
CLI](https://cli.github.com) and no inkentry install:

```bash
gh attestation verify inkentry-<version>-aarch64-apple-darwin.tar.gz --repo inkentries/inkentry
```

The signature is minted from a short-lived token GitHub issues to the release job
and recorded in a public transparency log, so unlike a checksum file it cannot be
regenerated by someone who has replaced the release assets.

The `v1.0.0-rc0` through `v1.0.0-rc2` archives predate both and have neither.
