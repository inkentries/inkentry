...
```bash
brew install spelunk-cloud/tap/spelunk
spelunk --version
```

<!-- TODO: confirm with Implementer — the Homebrew tap (spelunk-cloud/homebrew-tap)
     is not yet published by the release workflow (.github/workflows/release.yml
     ships tarballs only). Confirm the tap name + formula before 0.8.0. -->

### Install script (macOS and Linux)

Detects your OS/arch, downloads the matching release tarball, and installs to
`/usr/local/bin` (override with `SPELUNK_INSTALL_DIR`):

```bash
curl -fsSL https://spelunk.dev/install.sh | sh
spelunk --version
```

<!-- TODO: confirm with Implementer — install.sh is not yet generated by the
     release pipeline. Confirm the canonical URL (spelunk.dev/install.sh vs a
     raw GitHub URL) before 0.8.0. -->

### Debian / Ubuntu (`.deb`)

```bash
curl -fsSLO https://github.com/spelunk-cloud/spelunk/releases/latest/download/spelunk_0.8.0_amd64.deb
sudo dpkg -i spelunk_0.8.0_amd64.deb
spelunk --version
```

<!-- TODO: confirm with Implementer — the release workflow currently produces
     only .tar.gz artifacts. Confirm .deb packaging (cargo-deb / nfpm) and the
     exact artifact name before 0.8.0. -->

### Manual tarball (any platform)

If you can't use the options above, download the tarball for your platform from
the [releases page](https://github.com/spelunk-cloud/spelunk/releases) and put
...
Other targets (`x86_64-apple-darwin`, `aarch64-apple-darwin`,
`x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`) follow the same
pattern — swap the filename. Building from source? See [Building](building.md).
...
every source file, starts the bundled `spelunk-server` in the background (if one
isn't already running), and embeds your code so semantic search works out of the
box:
...
No config file, no Docker, no separate process to babysit. The next section
covers the always-available commands that work even before you index.
...
| Search | text + AST (`--mode text`) | + semantic search by meaning |
| Memory add/list/show | git-notes (local) | same |
| `spelunk explore` / `plan` | unavailable | available |
| Team memory sync | — | `spelunk memory push` / `pull` to a shared server |
...
By default the bundled `spelunk-server` provides embeddings and LLM inference.
If you'd rather run your own OpenAI-compatible endpoint (e.g. LM Studio on port
`1234`, Ollama on `11434`, or a vLLM proxy), point spelunk at it in
